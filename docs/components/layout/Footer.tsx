import React from 'react'
import './Footer.css'

const DiscordIcon = () => (
  <svg width="18" height="18" viewBox="0 0 127.14 96.36" fill="currentColor">
    <path d="M107.7,8.07A105.15,105.15,0,0,0,77.26,0a77.19,77.19,0,0,0-3.3,6.83A96.67,96.67,0,0,0,53.22,6.83,77.19,77.19,0,0,0,49.88,0,105.15,105.15,0,0,0,19.44,8.07C3.66,31.58-1.86,54.65,1,77.53A105.73,105.73,0,0,0,32,96.36a77.7,77.7,0,0,0,6.63-10.85,68.43,68.43,0,0,1-10.4-5c.87-.64,1.71-1.32,2.51-2a75.76,75.76,0,0,0,72.6,0c.8.7,1.64,1.38,2.51,2a68.43,68.43,0,0,1-10.4,5,77.7,77.7,0,0,0,6.63,10.85,105.73,105.73,0,0,0,31-18.83C129,54.65,122.93,31.58,107.7,8.07ZM42.45,65.69C36.18,65.69,31,60,31,53S36.18,40.36,42.45,40.36,53.83,46,53.83,53,48.72,65.69,42.45,65.69Zm42.24,0C78.41,65.69,73.24,60,73.24,53S78.41,40.36,84.69,40.36,96.07,46,96.07,53,91,65.69,84.69,65.69Z" />
  </svg>
)

const TwitterIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
  </svg>
)

const GithubIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
  </svg>
)

export interface FooterSocialLink {
  icon: 'discord' | 'twitter' | 'github' | React.ReactNode
  href: string
  label: string
}

export interface FooterLinkColumn {
  title: string
  links: Array<{
    label: string
    href: string
    external?: boolean
  }>
}

export interface FooterProps {
  logo?: React.ReactNode
  brandName?: string
  brandHref?: string
  description?: string
  socials?: FooterSocialLink[]
  linksGrid?: FooterLinkColumn[]
  copyright?: string
}

export const Footer: React.FC<FooterProps> = ({
  logo,
  brandName,
  brandHref,
  description,
  socials,
  linksGrid,
  copyright
}) => {
  const currentYear = new Date().getFullYear()
  const defaultCopyright = copyright !== undefined ? copyright : `\u00A9 ${currentYear} UNBRN. ALL RIGHTS RESERVED.`

  const renderSocialIcon = (icon: FooterSocialLink['icon']) => {
    if (typeof icon === 'string') {
      switch (icon) {
        case 'discord':
          return <DiscordIcon />
        case 'twitter':
          return <TwitterIcon />
        case 'github':
          return <GithubIcon />
        default:
          return null
      }
    }
    return icon
  }

  return (
    <footer className="unbrn-footer">
      <div className="footer-content container-1400">
        <div className="footer-brand">
          <a href={brandHref || '#'} className="footer-logo">
            {logo}
            {brandName && <span>{brandName}</span>}
          </a>
          {description && <p className="footer-description">{description}</p>}
          {socials && socials.length > 0 && (
            <div className="footer-socials">
              {socials.map((social, index) => (
                <a
                  key={index}
                  href={social.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="social-link"
                  aria-label={social.label}
                >
                  {renderSocialIcon(social.icon)}
                </a>
              ))}
            </div>
          )}
        </div>

        {linksGrid && linksGrid.length > 0 && (
          <div className="footer-links-grid">
            {linksGrid.map((col, index) => (
              <div key={index} className="footer-links-col">
                <div className="footer-col-title">{col.title}</div>
                {col.links.map((link, linkIndex) => (
                  <a
                    key={linkIndex}
                    href={link.href}
                    target={link.external ? "_blank" : undefined}
                    rel={link.external ? "noopener noreferrer" : undefined}
                  >
                    {link.label}
                  </a>
                ))}
              </div>
            ))}
          </div>
        )}
      </div>

      <div className="footer-bottom container-1400">
        {defaultCopyright && (
          <span className="footer-copyright">
            {defaultCopyright}
          </span>
        )}
      </div>
    </footer>
  )
}
